...
function openItemSheet(item: MenuItem): void {
  ...
  const prepHtml = item.prepTime ? `<span class="item-sheet-tag prep-time"><i class="ri-timer-line"></i> ${item.prepTime} mins</span>` : '';
      ...
      itemsHtml += `
        <div class="order-batch-group" style="margin-top:0.5rem; padding-top:0.5rem; border-top:1px dashed var(--clr-border);">
          <div style="display:flex; justify-content:space-between; margin-bottom:0.4rem;">
            ...
            <span style="color:var(--clr-gold); font-size:0.85rem; font-weight:600;">${timerText}</span>
          </div>
          ${gItems.map(it => `
